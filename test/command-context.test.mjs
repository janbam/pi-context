...
const AcquireCommand = "pi-context-acquire-command-context";
...
function createHarness({ dispatch = "immediate", idle = async () => {} } = {}) {
    ...
    const ctx = {
        sessionManager: {
            ...
            getBranch: () => entries,
            getEntries: () => entries,
            // Resumed session with ACM already enabled.
            getSessionState: () => ({ enabled: true }),
    ...
    const pi = {
        ...
        sendUserMessage: (content, options) => {
            ...
            if (content !== `/${AcquireCommand}`) return;
            assert.equal(options.expandPromptTemplates, true);
            if (dispatch === "throw") throw new Error("dispatch failed");
            if (dispatch === "immediate") void commands.get(AcquireCommand).handler("", commandCtx);
            if (dispatch === "async") queueMicrotask(() => commands.get(AcquireCommand).handler("", commandCtx));
        },
    };
    registerContext(pi);
    ...
    return {
        ...
        manual: (args = "enable") => commands.get("acm").handler(args, commandCtx),
...
test("manual /acm enable captures command ctx and avoids automatic dispatch", async () => {
...
test("/acm disable during pending acquisition settles it without scheduling compaction", async () => {
    const h = createHarness({ dispatch: "none" });
    const failed = assert.rejects(h.compact(), /cancelled before compaction started/);
    // The disable command's ctx resolves the pending acquisition; execute must see the new state.
    await h.manual("disable");
    await failed;
    await h.emit("turn_end");
    await h.emit("agent_end");
    await delay(10);
