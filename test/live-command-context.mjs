...
async function compactPhase(pi, phase, expectedAcquisitions, expectedSummaries, guard = false) {
    ...
    assert.equal(acquisitions.length, expectedAcquisitions, "real /acm dispatch count");
    ...
    assert.ok(!entries.some((entry) => entry.type === "message" && entry.message.role === "user" && text(entry.message).startsWith("/acm")), "internal command is not model history");
    const phaseEvents = pi.events.slice(offset);
...
try {
    ...
    await compactPhase(pi, "FIRST", 1, 1);
    const sessionFile = await compactPhase(pi, "CACHED", 1, 2);
    await pi.close();
    pi = startPi("resumed", sessionFile);
    await pi.request("set_auto_retry", { enabled: false });
    await compactPhase(pi, "RESUMED", 2, 3);
    await pi.close();
    pi = startPi("guard", undefined, true);
    await pi.request("set_auto_retry", { enabled: false });
    await compactPhase(pi, "GUARD", 1, 0, true);
