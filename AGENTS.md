...
*   **Scenario References**: `skills/context-management/references/` contains focused guidance for research, development/debugging, planning, repeated-item work, task switching, retry/pivot workflows, and Orca pi launch (`orca-pi-launch.md`).
...
*   **ACM Enablement**: `/acm` stores an `ExtensionCommandContext` required for `context_compact` navigation. `context_checkpoint` can label history directly, but `context_compact` needs command-context navigation support. Orca does not enable ACM by itself; prefix submitted pi prompts with `/acm ` (see `skills/context-management/references/orca-pi-launch.md`). Draft/prefill and resumed processes do not keep CommandCtx.
