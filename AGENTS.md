...
*   `src/acm.ts`: Durable ACM state — `pi-context.toml` config parsing, session-state validation, `/acm` argument mapping, new-session detection, the `<acm>` prompt section text, and the tool-loadout rule.
*   `src/context.ts`: `/context` command — TUI overlay dashboard showing token usage per category, estimated with Pi's `estimateTokens` over the session projection (the model's actual messages, including system messages) and scaled to actual usage.
...
*   **Compact flow**: `context_compact` returns `"compact start"` immediately → `turn_end` aborts the agent loop → `agent_end` defers (via `setTimeout` + `waitForIdle`, because `agent_end` fires before the agent is truly idle) → `didConversationAdvance` cancels if real entries arrived after the request → `branchWithSummary(tid)` (targets before the first system message, e.g. `root`, are moved to it by `anchorAfterInitialSystemMessage` so the prompt head stays first) creates the summary branch → leaf reset to `tid` so `navigateTree(nid)` rebuilds state → fire-and-forget continuation message. Treat compact as a phase-boundary operation, not mid-thought cleanup.
*   **ACM state**: `/acm` toggles durable session-global ACM state; `/acm enable` and `/acm disable` set it explicitly. `~/.pi/agent/pi-context.toml` may auto-enable only newly created sessions. The model sees ACM state through two channels: the durable tool loadout (`pi.setActiveTools`, synced on `session_start`, `session_tree`, and `/acm` transitions, because tree navigation restores the transcript's older loadout) and the constant `acm` prompt section set in `before_agent_start` (fork janbam/pi-mono#44 keeps it across triggered runs). Never edit per-run `selectedTools` or return `systemPrompt`; both break cache-stable triggered runs.
